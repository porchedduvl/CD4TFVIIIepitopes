...
Pp2+geom_violin(scale = "count",aes(factor(Antigen), fill = factor(Antigen))) + facet_grid(.~Donor) + scale_fill_manual(values = c("#009966","#CC3300")) + theme_minimal() 
...
#--------------------------------------------------------------
#plitting antigens samples
FVIII_1 <- DT$data$RUN_1_1179_FVIII
OVA_1 <- DT$data$RUN_1_1179_OVA

FVIII_2 <- DT$data$RUN_1_1195_FVIII
OVA_2 <- DT$data$RUN_1_1195_OVA

FVIII_3 <- DT$data$RUN_1_1207_FVIII 
OVA_3 <- DT$data$RUN_1_1207_OVA

FVIII_4 <- DT$data$RUN_3_1180_FVIII
OVA_4 <- DT$data$RUN_3_1180_OVA

DTFVIII <- rbind(FVIII_1, FVIII_2,FVIII_3,FVIII_4)
DTOVA <- rbind(OVA_1, OVA_2, OVA_3, OVA_4)

FVIIItotseq <- data.frame(DTFVIII$CDR3.aa)
write_csv(FVIIItotseq,"/mnt/NGS_YeastDisplay/Pipeline_NGS_TCR/_Valeria//FVIIItotseq.csv")

OVAtotseq <- data.frame(DTOVA$CDR3.aa)
write_csv(OVAtotseq,"/mnt/NGS_YeastDisplay/Pipeline_NGS_TCR/_Valeria//OVAtotseq.csv")
#-----------------------------------------------------
# CD3 lenght on frequency 
#FVIII


DTFVIII_f <- cbind(DTFVIII, frequence = DTFVIII$Clones/sum(DTFVIII$Clones)) 

Lenght <- nchar(DTFVIII_f$CDR3.aa)
ggplot(DTFVIII_f, aes(x= Lenght , y= DTFVIII_f$frequence))+geom_bar(stat="identity", width=0.7, fill="#009966")+xlim(5,25)+theme_minimal()+labs(subtitle = "Amino acid CDR3 length for FVIII")

#OVA
DTOVA_f <- cbind(DTOVA, frequence = DTOVA$Clones/sum(DTOVA$Clones)) 

Lenght <- nchar(DTOVA_f$CDR3.aa)
ggplot(DTOVA_f, aes(x= Lenght , y= DTOVA_f$frequence))+geom_bar(stat="identity", width=0.7, fill="#CC3300")+xlim(5,25)+theme_minimal()+labs(subtitle = "Amino acid CDR3 length for Ovalbumin")
...
#p1 = ggseqlogo( FVIII50align$CDR3.aa,seq_type='aa', method = 'bits', stack_width = 1) 
#p2 = ggseqlogo( FVIII50align$CDR3.aa, method = 'prob')

#print(p1) #plotting just one

#gridExtra::grid.arrange(p1, p2) #to plot the 2 together 

#-------------------------------------------------------------------------------------------
#VDJ pairing FVIII

#prep matrice for Chord diagram
listVF <- sort(unique(unlist(strsplit(unique(DTFVIII_f$V.name), ", "))))
listJF <- sort(unique(unlist(strsplit(unique(DTFVIII_f$J.name), ", "))))
...
for (v in (1:nrow(VJFVIII))){
  for (j in (1:ncol(VJFVIII))){
    for (a in (1:nrow(DTFVIII_f))){
      if ((listVF[[v]] %in% unlist(strsplit(unique(DTFVIII_f$V.name[a]), ", "))) & (listJF[[j]] %in% unlist(strsplit(unique(DTFVIII_f$J.name[a]), ", ")))){
...
#prep matrice for Chord diagram
listVO <- sort(unique(unlist(strsplit(unique(DTOVA_f$V.name), ", "))))
listJO <-sort(unique(unlist(strsplit(unique(DTOVA_f$J.name), ", "))))
...
for (v in (1:nrow(VJOVA))){
  for (j in (1:ncol(VJOVA))){
    for (a in (1:nrow(DTOVA_f))){
      if ((listVO[[v]] %in% unlist(strsplit(unique(DTOVA_f$V.name[a]), ", "))) & (listJO[[j]] %in% unlist(strsplit(unique(DTOVA_f$J.name[a]), ", ")))){
...
col = c('TRBV10-1'="#FF0033",'TRBV10-2'="#FFCCCC",'TRBV11-2'="#FF3300",
        ...
        'TRBV6-6'="#CC6699", 'TRBV7-1'="#993366",'TRBV7-3'="#CC0033",'TRBV7-8'="#993366",
...
intersect(DTFVIII$CDR3.aa, DTFVIII5$CDR3.aa)  #509 public clones 
intersect(DTFVIII_f$CDR3.aa, DTOVA_f$CDR3.aa) #165 public clones 
intersect(DT$data$RUN_1_1179_FVIII$CDR3.aa, DT$data$RUN_1_1195_FVIII$CDR3.aa, DT$data$RUN_1_1207_FVIII$CDR3.aa, DT$data$RUN_3_1180_FVIII$CDR3.aa)
intersect(DTFVIII5$CDR3.aa, DTpool5$CDR3.aa) #2958 public clones 
intersect(DTFVIII5$CDR3.aa, DTOVA_f$CDR3.aa) #386 public clones
intersect(DTpool5$CDR3.aa, DTOVA_f$CDR3.aa)  #362 public clones
intersect(DTpool5$CDR3.aa, DTFVIII_f$CDR3.aa) #225 public clones
intersect(DTpool5$CDR3.aa, DT50FVIII$CDR3.aa) #25 public clones
